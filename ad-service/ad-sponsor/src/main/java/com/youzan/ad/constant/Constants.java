package com.youzan.ad.constant;

/**
 * 指定关于错误常量
 *
 * @Author TCP
 * @create 2019/3/27 10:56
 */
public class Constants {
    public class ErrorMsg {
        public static final String REQUEST_ILLEGAL_PARAMETERS = "请求参数不合法";
        public static final String USER_NAME_ALREADY_EXIST = "用户名已存在";
        public static final String CAN_NOT_FIND_USER = "无此用户";
        public static final String DATE_FORMAT_ERROR = "日期转换错误";
        public static final String PLAN_NAME_ALREADY_EXIST = "计划名称已经存在";
        public static final String CAN_NOT_FIND_PLAN = "当前计划不存在";
    }
}
